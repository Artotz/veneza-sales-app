...
export default function LDVScreen({ navigation }: any) {
  ...
  function renderInputField(field: string) {
    switch (field) {
      case 'fillFactor':
        return (
          <View
      ...
      case 'totalTime':
        const totalSeconds = parseInt(values[field] ?? '0', 10) || 0;
        const minutesCheck = Math.floor(totalSeconds / 60);
        const secondsCheck = totalSeconds % 60;

        const minutes = minutesCheck === 0 ? '' : minutesCheck.toString();
        const seconds =
          secondsCheck === 0 ? (minutesCheck === 0 ? '' : '0') : secondsCheck.toString();

        const handleChangeTotalTime = (newMinutes: string, newSeconds: string) => {
          if (newSeconds.length > 2) return;
          const m = parseInt(newMinutes || '0', 10);

          const secondsParsed = parseInt(newSeconds || '0', 10);
          if (isNaN(secondsParsed)) return;
          const s = secondsParsed < 60 ? secondsParsed : 59;

          const total = m * 60 + s;
          handleChange(field, total.toString());
        };
          ...
          <View
            ...
            <View className="flex-row gap-3">
              <View className="flex-1">
                ...
                <TextInput
                  className="rounded-xl border border-gray-300 bg-gray-50 p-3 text-center text-[#231f1e]"
                  placeholder={t('prodCalc.minutes')}
                  placeholderTextColor="#888"
                  value={minutes}
                  onChangeText={(text) => handleChangeTotalTime(text, seconds)}
                  keyboardType="numeric"
                />
              </View>

              <View className="flex justify-end pb-1.5">
                <Text className="text-4xl font-semibold text-[#231f1e]">:</Text>
              </View>
              ...
              <View className="flex-1">
                ...
                <TextInput
                  className="rounded-xl border border-gray-300 bg-gray-50 p-3 text-center text-[#231f1e]"
                  placeholder={t('prodCalc.seconds')}
                  placeholderTextColor="#888"
                  value={seconds}
                  onChangeText={(text) => handleChangeTotalTime(minutes, text)}
                  keyboardType="numeric"
                />
              </View>
            </View>
